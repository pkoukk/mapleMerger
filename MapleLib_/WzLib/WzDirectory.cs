    ...
    public class WzDirectory : WzObject
    {
        ...
        internal void ParseDirectory(bool lazyParse = false)
        {
            ...
            for (int i = 0; i < entryCount; i++)
            {
                    ...
                    case (byte) WzDirectoryType.RetrieveStringFromOffset_2:
                        {
                            ...
                            reader.SetOffsetFromFStartToPosition(stringOffset); // set to the offset position and read from

                            reader.PrintHexBytes(50); //  04 EE FA E7 F3 E5 C7 CB D4 D4 DC E7 D5 D9 D3 C4 96 D0 D7 DC 80 A5 55 48 00 80 55 F4 0B 26 36 0E 2E 1A 04 F4 CD C7 C3 CF CF C3 FA E1 9C DA D9 D2 80 33 
                ...
                {
                    WzImage img = new WzImage(fname, reader, checksum, unk_GMS230)
                    {
                        BlockSize = fsize,
                        Offset = offset,
                        Parent = this
        ...
        internal void SaveDirectory(WzBinaryWriter writer)
        {
            ...
            foreach (WzImage img in images)
            {
                if (!writer.WriteWzObjectValue(img.name, WzDirectoryType.WzImage_4, wzFile.Is64BitWzFile ? (byte)img.unk_GMS230 : Byte.MaxValue))  // true if written as an offset
                {
                }
            ...
            foreach (WzDirectory dir in subDirs)
            {
                if (!writer.WriteWzObjectValue(dir.name, WzDirectoryType.WzDirectory_3, wzFile.Is64BitWzFile ? (byte) 0 : Byte.MaxValue)) // true if written as an offset
                {
                }
